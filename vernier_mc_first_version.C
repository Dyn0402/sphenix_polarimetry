...
//  const float DX1 = 0.0; // mm
  const float DX1 = 1.0; // mm
//  const float DY1 = 0.0; // mm
  const float DY1 = 1.0; // mm
...
//    const float BETA = 3000; // mm
  ...
  const float zBBC = 1440;    // mm, BBC position
...
float GetDensity(int ibeam, float zz)
{
  ...
  Double_t ztmp, atmp;
  if( first ) {
    TFile* fb = new TFile("from_kawall/prof_blue.root");
    TGraph* grb = (TGraph*)fb->Get("Graph");
    nn = grb->GetN();
    for( int i=0; i<nn; i++ ) {
      grb->GetPoint(i,ztmp,atmp);
      a1[i] = atmp;
    }
    fb->Close();

    TFile* fy = new TFile("from_kawall/prof_yell.root");
    TGraph* gry = (TGraph*)fy->Get("Graph");
    if( nn<2 || nn!=gry->GetN() ) {
      printf("Error in GetDensity() when reading graphs: %d %d\n",nn,gry->GetN());
    }
    for( int i=0; i<nn; i++ ) {
      gry->GetPoint(i,ztmp,atmp);
      a2[i] = atmp;
    }
    fy->Close();
